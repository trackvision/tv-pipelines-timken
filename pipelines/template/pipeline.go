...
func setupDAGEdges(j *goflow.Job) {
	j.SetDownstream(j.Task("fetch_data"), j.Task("process_data"))
	j.SetDownstream(j.Task("process_data"), j.Task("save_results"))
}
...
func (p *Pipeline) RunOnce() error {
	...
	ops := []struct {
		...
		{"fetch_data", &FetchDataOp{pipeline: p}},
		{"process_data", &ProcessDataOp{pipeline: p}},
		{"save_results", &SaveResultsOp{pipeline: p}},
	}
...
func (o *FetchDataOp) Run() (interface{}, error) {
	...
	// Example:
	// data, err := tasks.FetchData(ctx, o.pipeline.config.ExampleAPIURL, id)
	// if err != nil {
...
func (o *SaveResultsOp) Run() (interface{}, error) {
	...
	// }
	// result, err := tasks.SaveToDirectus(ctx, o.pipeline.state.DirectusClient, processedData)
	// if err != nil {
